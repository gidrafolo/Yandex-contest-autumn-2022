...
class Question:
    ...
    def __init__(self, count_A, count_B, potion_number, answer):
...
def main():
    ...
    while i < int(question_count):
        ...
        answer_list.append(findAnswer(Question(*question_by_array, False), validated_recipe_list))
        i += 1
    ...
    while i < len(answer_list):
        resultStr += '1' if answer_list[i] else '0'
        i += 1
...
def object_input(recipes_list, question_list):
    ...
    while qIndex < len(question_list):
        question_list[qIndex].answer = findAnswer(question_list[qIndex], validated_recipe_list)
...
def checkRecipe(recipeIndex, loopCheckerList, recipes_list, validated_recipe_list):
    ...
    for ingredientIndex in recipes_list[recipeIndex].list_of_ingred:  # получаем cписок игридиентов (их индекс - 1)
        ...
        if ingredient_recipe.isValid:
            current_validating_recipe.allCountA = current_validating_recipe.allCountA + ingredient_recipe.allCountA
            current_validating_recipe.allCountB = current_validating_recipe.allCountB + ingredient_recipe.allCountB
        else:
            current_validating_recipe.isValid = False
            break
